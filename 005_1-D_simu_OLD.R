...
B <- A * wave_v4(h = H, delta = delta, A = 1) 
# [1:20, 1:20]
quantile(B)
...
quantile(C11)
#       0%       25%       50%       75%   100% 
# 0.1073797 0.4060058 0.6626273 0.8780986  1.0000000 
    ...
    for(r in seq(3, p, by = 1)) {
      ...
      for (c in seq(1, (r-1), by = 1)) {
        ...
        for (t in c(PN)) {
          ...
          delta <- sample(seq(-1, 1, by = 0.1), 1)
          B_rt <- A * wave_v4(h = H, delta = delta, A = 1)
...
plt_Sig(log(SIGMA_1212), p = 2) #Warning message:
# In log(SIGMA_1212) : NaNs produced
range(SIGMA_1212)
quantile(SIGMA_1212)
#       0%        25%      50%        75%        100% 
# -9.4104396  0.7350776   3.1256213  7.2399777   65.7860876 

plt_Sig(log(SIGMA_1313), p = 3)
plt_Sig(log(SIGMA_1414), p = 4)
plt_Sig(log(SIGMA_1515), p = 5)
plt_Sig(log(SIGMA_1616), p = 6)

quantile(SIGMA_1616)

plt_Sig(log(abs(SIGMA_1313)), p = 3)
plt_Sig(log(abs(SIGMA_1414)), p = 4)
plt_Sig(log(abs(SIGMA_1515)), p = 5)
plt_Sig(log(abs(SIGMA_1616)), p = 6)
...
#-------
quantile(SIGMA_1717)
plt_Sig(log(abs(SIGMA_1717)), p = 7)
...
quantile(SIGMA_Chain_7)
plt_Sig(log(abs(SIGMA_Chain_7)), p = 7)
...
png(paste0(image_path, "SIGMA7_Chain_Inv_Ture2.png"),
    res = 300, width = 8, height = 7, units = "in")
plt_Sig(SIGMA_7_C_inv_mat, p = 7)
dev.off()
...
plt_Sig(SIGMA_7_C_inv_mat, p = 7)
plt_Sig(log(abs(SIGMA_7_C_inv_mat)), p = 7)
quantile(SIGMA_7_C_inv_mat)

Test_sym_pd(SIGMA_7_C_inv_mat)
#[1] "Symmetric: Yes"
#[1] "p.d.: Yes"
