...
# Aim:
  # Test the p.d. of Wendland function (k = 3/2)
  # featured as rapid decrease symmetrically about origin h = 0
  # and smooth decay

# Expect:
  # SIGMA_inv is p.d. for all combinations, numerical stable


# Method:
  # modify TST6 function to use Wendland (k = 3/2, R = 0.5)


# Result:
    #Use Wendland function 
    # SIGMA_inv all p.d.




TST7_Pert_build_SG_SGInv <- function(p, data, A_mat, dlt_mat, sig2_mat, kappa_mat, d_vec, h) {
...
for (dlt in seq(0.5, 1, by = 0.2)){
  ...
  for (a in seq(0.5, 1, by = 0.1)){
    ...
    SG_SG_inv_5 <- TST7_Pert_build_SG_SGInv(p = 5, data = hierarchy_data, 
