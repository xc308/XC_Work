...
Tst_Fit_Obs_indx <- Fn_Tst_Fit_Obs_indx(df = df_WL, num_folds = 4)
...
for (f in 1:num_folds) {
  ...
  df_cokrig <- cokrig(optm_pars_vec = optm_pars_WL, all_pars_lst = all_pars_lst_CAR_6,
                      p = 6, data_str = hierarchy_data6, phi = phi, H_adj = H_adj, 
                      h = H, b = "Wendland", obs_indx = obs_indx, df = df_WL)
